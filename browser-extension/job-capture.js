(function (root, factory) {
  ...
  const selectors = {
    linkedin: {
      ...
      description: [".jobs-description__content", ".jobs-box__html-content", "#job-details", ".jobs-search__job-details--container [class*='description']", ".scaffold-layout__detail [class*='description']", "[class*='jobs-description']"],
  ...
  function extract(document, url) {
    ...
    if (missing.length) throw new Error(`Adapter 0.5.2 could not identify: ${missing.join(", ")}. Wait for the selected job panel to finish loading and try again.`);
