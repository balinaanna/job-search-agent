(function (root) {
  ...
  function fieldLabel(field) {
    ...
    return normalize(explicit?.innerText || wrapping?.innerText || field.getAttribute("aria-label") || field.getAttribute("name") || field.getAttribute("placeholder") || "");
  }
  ...
  function matchAnswers(answers, fields) {
    ...
    for (const answer of answers) {
      ...
      for (const field of fields) {
        ...
        const score = similarity(answer.question, fieldLabel(field));
        if (!best || score > best.score) best = { field, score, label: fieldLabel(field) };
      }
      if (best && best.score >= 0.34) { used.add(best.field); matches.push({ answer, ...best }); }
  ...
  root.JobAgentMatcher = { normalize, similarity, matchAnswers, eligibleFields, fieldLabel };
})(globalThis);
