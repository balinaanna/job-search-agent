...
test("server-renders the job-search workspace", async () => {
  ...
  assert.match(html, /Email alerts and manual import/);
  assert.match(html, /LinkedIn/);
  assert.match(html, /Indeed/);
  assert.match(html, /Eluta/);
  assert.match(html, /Connect Gmail alerts/);
...
test("dashboard uses all exported workflow data and responsive styling", async () => {
  ...
  assert.match(jobsView, /Show 15 more/);
  ...
  assert.match(jobsView, /Needs capture/);
  assert.match(alertInbox, /ADD JOBS/);
  assert.match(alertInbox, /unified workspace/);
  assert.doesNotMatch(alertInbox, /alert-jobs/);
  assert.match(jobsView, /Filter by source/);
  assert.match(jobsView, /workflow-drawer/);
