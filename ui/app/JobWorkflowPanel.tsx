...
const recommendationLabels:Record<string,string>={strong_apply:"Strong match",apply:"Apply",selective_apply:"Stretch",do_not_apply:"Pass"};
const sourceLabels:Record<string,string>={linkedin:"LinkedIn",indeed:"Indeed",eluta:"Eluta",greenhouse:"Greenhouse"};
function readable(value:string|undefined){return (value||"Not started").replaceAll("_"," ").replace(/\b\w/g,letter=>letter.toUpperCase());}
...
export function JobWorkflowPanel({job,onClose}:{job:Job;onClose:()=>void}){
  ...
  async function interview(){setShowPrep(value=>!value);if(prep)return;setPrepMessage("Preparing from verified evidence…");try{let response=await fetch(`http://localhost:8787/api/jobs/${job.id}/interview-preparation`);if(response.status===404)response=await fetch(`http://localhost:8787/api/jobs/${job.id}/interview-preparation`,{method:"POST"});const value=await response.json();if(!response.ok)throw new Error(value.error);setPrep(value);setPrepMessage("");}catch(error){setPrepMessage(error instanceof Error?error.message:"Interview preparation unavailable.");}}
  return <section className="job-workflow-detail active-role-workflow">
    <header><div><span>JOB DETAILS & APPLICATION</span><h3>{job.role}</h3><p>{job.company} · {job.profileStale?"Fit needs refresh":job.stage==="analyzed"?"Analyzed":"Ready to analyze"}</p></div><div><button onClick={archive}>{showPosting?"Hide saved posting":"View saved posting"}</button>{job.stage==="analyzed"&&<button onClick={interview}>{showPrep?"Hide interview prep":"Interview prep"}</button>}<a href={job.postingUrl} target="_blank" rel="noreferrer">Original link</a><button onClick={onClose}>Close</button></div></header>
    <section className="workflow-job-overview">
      <div className="workflow-fit-summary"><strong>{job.stage==="analyzed"?`${job.fitScore??0}%`:`${job.discoveryScore??0}%`}</strong><div><span>{job.profileStale?"Previous fit":job.stage==="analyzed"?"Verified fit":"Preliminary match"}</span><b>{job.recommendation?recommendationLabels[job.recommendation]||readable(job.recommendation):"Awaiting analysis"}</b></div></div>
      <dl><div><dt>Company</dt><dd>{job.company}</dd></div><div><dt>Location</dt><dd>{job.location||"Not stated"}</dd></div><div><dt>Source</dt><dd>{sourceLabels[job.source||""]||readable(job.source)}</dd></div><div><dt>{job.postedDate?"Posted":"First seen"}</dt><dd>{date(job.postedDate||job.firstSeenAt)}</dd></div></dl>
      {job.reasons?.length&&<section><strong>Why it may fit</strong><ul>{job.reasons.map(reason=><li key={reason}>{reason}</li>)}</ul></section>}
      {job.risk&&<section className="workflow-main-risk"><strong>Main consideration</strong><p>{job.risk}</p></section>}
      {job.nextAction&&<section className="workflow-next-action"><strong>Recommended next action</strong><p>{job.nextAction}</p></section>}
    </section>
