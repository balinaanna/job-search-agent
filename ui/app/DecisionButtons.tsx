...
type AnswerPlan = { answers: Array<{ question_id: string; question: string; category: string; status: string; proposed_answer: string | null; evidence_ids: string[]; reason: string }>; submission_authorized: boolean };
...
function ApplicationAnswersAction({ leadId, initialStatus, onStatus }: { leadId: string; initialStatus: string; onStatus: (status: string) => void }) {
  ...
  useEffect(() => { if (["application_answers_requested", "application_answers_running", "application_answers_failed"].includes(initialStatus)) fetch(`http://localhost:8787/api/jobs/${leadId}/workflow`).then((r) => r.ok ? r.json() : null).then((latest) => latest?.id && setRun(latest)).catch(() => undefined); if (initialStatus === "application_answers_completed") void loadPlan(); }, [leadId, initialStatus]);
  ...
  return <section className="application-package"><header><div><span>ANSWER PLAN</span><strong>Review required</strong></div><b>{plan.answers.length} questions</b></header><div className="answer-list">{plan.answers.map((answer) => <article key={answer.question_id}><div><strong>{answer.question}</strong><span>{answer.category.replaceAll("_", " ")} · {answer.status.replaceAll("_", " ")}</span></div>{answer.proposed_answer ? <p>{answer.proposed_answer}</p> : <p>{answer.reason}</p>}{answer.evidence_ids.length > 0 && <small>Evidence: {answer.evidence_ids.join(", ")}</small>}</article>)}</div><small>No answers have been entered or submitted. Flagged questions require your direct answer; every draft will need approval.</small></section>;
}
