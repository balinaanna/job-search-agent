...
export function DecisionButtons({ leadId }: { leadId: string }) {
  ...
  if (["application_answers_approved", "form_filling_started", "submission_review_required"].includes(status)) return <FormFillAction leadId={leadId} initialStatus={status} onStatus={setStatus} />;
...
function FormFillAction({ leadId, initialStatus, onStatus }: { leadId: string; initialStatus: string; onStatus: (status: string) => void }) {
  ...
  if (initialStatus === "submission_review_required") return <section className="application-package"><header><div><span>FINAL REVIEW</span><strong>Submission review required</strong></div><b>Stopped safely</b></header><p>All fields and documents were marked complete. Nothing has been submitted.</p><a href={sourceUrl} target="_blank" rel="noreferrer">Return to employer application</a><small>The next stage will perform the final review and ask for separate, explicit permission before any Submit action.</small></section>;
