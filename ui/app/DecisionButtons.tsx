...
export function DecisionButtons({ leadId }: { leadId: string }) {
  ...
  if (["application_answers_approved", "form_filling_started", "submission_review_required", "submission_authorized"].includes(status)) return <FormFillAction leadId={leadId} initialStatus={status} onStatus={setStatus} />;
...
function FormFillAction({ leadId, initialStatus, onStatus }: { leadId: string; initialStatus: string; onStatus: (status: string) => void }) {
  ...
  if (initialStatus === "submission_authorized") return <section className="application-package"><header><div><span>SUBMISSION AUTHORIZED</span><strong>Permission recorded</strong></div><b>Not submitted</b></header><p>{packageData?.package.company} · {packageData?.package.role}</p><div className="package-integrity"><strong>Authorization is scoped to this application</strong><span>The form and documents were reviewed. No submit action has occurred.</span></div><a href={sourceUrl} target="_blank" rel="noreferrer">Return to employer application</a><small>The next stage may execute the final Submit action and must record the employer's confirmation result.</small></section>;
