...
type BrowserFillReport = { filled: Array<{ question_id: string; label: string }>; unmatched: Array<{ question_id: string; question: string }>; blockers: string[]; submit_clicked: boolean };
...
function FormFillAction({ leadId, initialStatus, onStatus }: { leadId: string; initialStatus: string; onStatus: (status: string) => void }) {
  ...
  if (initialStatus === "application_answers_approved") return <section className="application-package"><header><div><span>ASSISTED APPLICATION</span><strong>Ready to fill</strong></div><b>No submission</b></header><p>Open the employer form and use the approved answers and documents. The agent will stop before submission.</p><div className="form-import"><input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://employer.example/apply" /><button disabled={busy || !sourceUrl.trim()} onClick={start}>{busy ? "Starting…" : "Open application workspace"}</button></div>{message && <small role="status">{message}</small>}</section>;
  if (initialStatus === "application_submitted") return <section className="application-package submission-success"><header><div><span>APPLICATION SUBMITTED</span><strong>Confirmation recorded</strong></div><b>Applied</b></header><p>{packageData?.package.company} · {packageData?.package.role}</p><div className="package-integrity"><strong>Application workflow complete</strong><span>The job lead is now marked applied and protected from duplicate applications.</span></div></section>;
  if (initialStatus === "submission_blocked") return <section className="application-package"><header><div><span>SUBMISSION BLOCKED</span><strong>Stopped for attention</strong></div><b>Not applied</b></header><p>{packageData?.package.company} · {packageData?.package.role}</p><div className="package-integrity"><strong>No successful submission was recorded</strong><span>The blocking message was saved. Do not assume the employer received the application.</span></div><a href={sourceUrl} target="_blank" rel="noreferrer">Inspect employer application</a></section>;
