...
type AnswerPlan = { answers: Array<{ question_id: string; question: string; category: string; status: string; proposed_answer: string | null; evidence_ids: string[]; reason: string }>; answers_approved?: boolean; submission_authorized: boolean };
...
export function DecisionButtons({ leadId }: { leadId: string }) {
  ...
  if (["form_questions_saved", "application_answers_requested", "application_answers_running", "application_answers_failed", "application_answers_completed", "application_answers_approved"].includes(status)) return <ApplicationAnswersAction leadId={leadId} initialStatus={status} onStatus={setStatus} />;
