...
function FormFillAction({ leadId, initialStatus, onStatus }: { leadId: string; initialStatus: string; onStatus: (status: string) => void }) {
  ...
  async function start() { setBusy(true); try { const response = await fetch(`http://localhost:8787/api/jobs/${leadId}/form-fill`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_url: sourceUrl }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "Assisted form filling could not start."); onStatus(payload.status); window.open(sourceUrl, "_blank", "noopener,noreferrer"); } catch (error) { setMessage(error instanceof TypeError ? "Workflow service is offline." : error instanceof Error ? error.message : "Assisted form filling could not start."); } finally { setBusy(false); } }
